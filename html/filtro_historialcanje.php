...
<!--tablas-->
        ...
        <section class="table__body">
            <table class="tabla">
                ...
                <tbody class="icono">
                        <?php
                        if($validacion==false  ){
                            $consulta="SELECT * FROM historial_canje WHERE nit_empresa='$_SESSION[empresa]' and cedula_cliente='$_SESSION[user]' and usuario_actualizo is null ";
                            ...
                            <tr>
                                ...
                                <td><?php echo $historial['nit_empresa'] ?></td>
                                <td><?php echo $historial['cedula_cliente'] ?></td>
                                <td><?php echo $historial['total_puntos_canjeados'] ?></td>
                        ...
                        <?php }}else {
                            if($_POST["buscar"]==''){
                                ...
                                $consulta="SELECT * FROM historial_canje WHERE nit_empresa='$_SESSION[empresa]' and cedula_cliente='$_SESSION[user]' and usuario_actualizo is null  and  id='$_POST[buscar]' ";
                                $resultado=mysqli_query($con,$consulta);
                                while ($historial=mysqli_fetch_array($resultado)){
                                    ?>
                                    <tr>
                                        <td><?php echo $historial['numero_factura'] ?></td>
                                        <td><?php echo $historial['nit_empresa'] ?></td>
                                        <td><?php echo $historial['cedula_cliente'] ?></td>
                                        <td><?php echo $historial['valor_compra'] ?></td>
                                        <td><?php echo $historial['fecha']; ?></td>
                                        <td><?php echo $historial['estado'] ?></td>
                                    <td>
                                        <button class="btn btn-sm btn-success seleccionar">
    ...
    <div class="consultas desplegar" id="consultar">
        <form class="form-control w-50" action="../html/historial_canje.php" method="post">
            ...
            <div class="form-control border-white">
                <label for="">puntos canjeados</label>
                <input type="number" readonly class="form-control w-90 rell" name="puntos_canjeados">
            </div>

            <div class="form-control border-white d-flex justify-content-evenly">
                <label for="">fecha de canje</label>
                <input type="date" readonly class="form-control w-25 rell" name="fecha_canje">
